...
describe('AuthService', () => {
  ...
  it('should return token when user login', async() => {
    ...
    const response = await AuthService(mockAxios).login({ email: 'lele@lele.com', password: '1234' })
    expect(response.data).toHaveProperty('token')
    expect(response).toMatchSnapshot()
  })

  it('should return a user when user register', async () => {
    const user = { name: 'Lele', password: '1234', email: 'lele@lele.com' }
    mockAxios.post.mockImplementationOnce(() => {
  ...
  it('should throw an error when there is no network', async () => {
